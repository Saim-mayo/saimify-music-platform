...
const handleAudioDelivery = async (req, res, mode = 'stream') => {
   ...
   if (hasRange) {
      ...
      // Explicitly match and capture the start and end byte positions
   ...
   const end = requestedEnd !== null
      ...
      : (statusCode === 206 ? Math.min(start + CHUNK_SIZE - 1, fileSize - 1) : fileSize - 1);
   ...
   const stream = await axios(requestConfig);
...
const createSong = asyncHandler(async (req, res) => {
   ...
   // Notify the uploading artist about their upload
   createNotification({
      userId: req.user.userId,
      type: 'song_uploaded',
      title: 'Song uploaded',
      message: `Your song "${music.title || 'Untitled'}" has been uploaded and is ${music.status}.`,
      data: { songId: music._id, status: music.status }
   });
...
const updateSong = asyncHandler(async (req, res) => {
   ...
   const music = await updateSongService({
...
// Authorize a stream before a native audio element requests its bytes.
const canStreamSong = asyncHandler(async (req, res) => {
...
const searchSongs = asyncHandler(async (req, res) => {
   ...
   const songs = genre
      ? await searchSongsService(query, genre)
      : await searchSongsService(query);
...
const createAlbum = asyncHandler(async (req, res) => {
   ...
   // Notify the artist about the new album
   createNotification({
      userId: req.user.userId,
      type: 'album_created',
      title: 'Album created',
      message: `Your album "${album.title || 'Untitled'}" has been created and is ${album.status}.`,
      data: { albumId: album._id, status: album.status }
   });
...
const getAllAlbums = asyncHandler(async (req, res) => {
   ...
   const result = await getAllAlbumsService({
      page: parseInt(req.query.page, 10) || 1,
      limit: parseInt(req.query.limit, 10) || 10
   });
