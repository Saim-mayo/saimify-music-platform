...
const HMAC_SECRET = env.REFRESH_TOKEN_HMAC_SECRET;
const EXPIRY_MS = limits.refreshTokenTtlMs;
 ...
 */
async function rotateTokenInternal(rawIncoming) {
    const incomingHash = hashToken(rawIncoming);

    const newRaw = crypto.createHmac('sha256', HMAC_SECRET).update(`${rawIncoming}:rotation`).digest('hex');
    const replacementHash = hashToken(newRaw);
    ...
    const existing = await RefreshToken.findOneAndUpdate(
        ...
        { $set: { used: true, usedAt: new Date(), replacementHash } },
...
async function rotateToken(rawIncoming) {
    const incomingHash = hashToken(rawIncoming);
    const pending = pendingRotations.get(incomingHash);
    if (pending) return pending;

    const rotation = rotateTokenInternal(rawIncoming);
    pendingRotations.set(incomingHash, rotation);

    try {
        return await rotation;
    } finally {
        if (pendingRotations.get(incomingHash) === rotation) {
            pendingRotations.delete(incomingHash);
        }
    }
}
/**
 ...
 * Wipes every refresh token for the user (all devices/sessions), not
 * just one family — a password reset should end every existing session,
 * not just the one that requested it.
 */
async function revokeAllSessions(userId) {
    await RefreshToken.deleteMany({ userId });
}

module.exports = { createTokenFamily, rotateToken, revokeFamily, revokeAllSessions, hashToken };