...
const populateQueueDocument = async (queueDoc) => {
   if (!queueDoc) return queueDoc
   return queueDoc.populate({
      path: 'queue',
      select: 'title artist artistName coverUrl visibility',
      populate: { path: 'artist', select: 'username avatar' }
   })
}

const populateQueueArtistData = async (queueDoc) => {
   if (!queueDoc) return queueDoc
   return queueDoc.populate({
      path: 'queue',
      select: 'title artist artistName coverUrl visibility',
...
const addToQueueService = async ({ userId, songId }) => {
   ...
   return populateQueueDocument(queueDoc);
};
...
const nextSongService = async (userId) => {
   ...
   await queueDoc.save();
   await populateQueueDocument(queueDoc);

   return {
      currentIndex: queueDoc.currentIndex,
      song: queueDoc.queue[queueDoc.currentIndex]
   };
};
...
const setQueueService = async ({ userId, queue, currentIndex = 0 }) => {
   if (!Array.isArray(queue) || queue.length === 0) {
      throw new AppError('Queue must be a non-empty array', 400);
   }

   if (!queue.every((songId) => mongoose.Types.ObjectId.isValid(songId))) {
      throw new AppError('Queue items must be valid song IDs', 400);
   }

   const validCount = await MusicModel.countDocuments({ _id: { $in: queue } });
   ...
   if (!queueDoc) {
      ...
      queueDoc.queue = queue;
      queueDoc.currentIndex = safeIndex;
      await queueDoc.save();
   }
