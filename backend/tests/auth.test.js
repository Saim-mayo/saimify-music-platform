...
describe('Refresh token rotation', () => {
   ...
   it('allows two near-simultaneous refresh requests to share the same replacement token', async () => {
      await register();
      await verifyEmail();
      const loginRes = await login();
      const oldRefreshToken = loginRes.body.refreshToken;
      ...
      const [firstRes, secondRes] = await Promise.all([
         request(app).post('/api/auth/refresh-token').send({ refreshToken: oldRefreshToken }),
         request(app).post('/api/auth/refresh-token').send({ refreshToken: oldRefreshToken })
      ]);

      expect([firstRes.status, secondRes.status]).toEqual(expect.arrayContaining([200]));
      const successful = [firstRes, secondRes].find((res) => res.status === 200);
      expect(successful).toBeDefined();
      const other = [firstRes, secondRes].find((res) => res.status !== 200);
      if (other) {
         expect(other.status).toBe(200);
      }
      expect(firstRes.body.refreshToken).toBeDefined();
      expect(secondRes.body.refreshToken).toBeDefined();
      expect(firstRes.body.refreshToken).toBe(secondRes.body.refreshToken);

      const activeTokens = await RefreshToken.find({ userId: loginRes.body.user.id, used: false });
      expect(activeTokens.length).toBe(1);
   });

   it('still revokes the family when a rotated token is replayed after the grace window', async () => {
      await register();
      await verifyEmail();
      const loginRes = await login();
      const oldRefreshToken = loginRes.body.refreshToken;
      ...
      const usedToken = await RefreshToken.findOne({ tokenHash: require('../src/utils/tokenStore').hashToken(oldRefreshToken) });
      expect(usedToken).toBeDefined();
      usedToken.usedAt = new Date(Date.now() - 60 * 1000);
      ...
      expect(replayRes.status).toBe(403);
      const activeTokens = await RefreshToken.find({ userId: loginRes.body.user.id });
