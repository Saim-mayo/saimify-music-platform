...
describe('POST /api/webhook — checkout.session.completed', () => {
   it('activates the plan and creates a Payment row on first checkout', async () => {
      ...
      const notifications = await Notification.find({ 'data.stripeEventId': 'evt_checkout_1' });
...
describe('POST /api/webhook — invoice.paid', () => {
   it('creates a Payment row and syncs the subscription status on invoice payment', async () => {
      ...
      const notification = await Notification.findOne({ 'data.invoiceId': 'in_test_123' });
      expect(notification).toBeTruthy();
      expect(notification.type).toBe('payment_success');
      expect(notification.message).toBe('Your pro subscription payment was received.');
   ...
   it('creates a prorated payment notification for subscription updates with a billing reason', async () => {
      await seedPlan();
      await createUser({
         subscription: {
            plan: 'pro',
      ...
      const notification = await Notification.findOne({ 'data.invoiceId': 'in_test_456' });
      expect(notification).toBeTruthy();
      expect(notification.type).toBe('payment_success');
      expect(notification.message).toContain('charged $2.50 for prorated usage');
   });

   it('does not create duplicate payment notifications for repeated invoice.paid deliveries', async () => {
      await seedPlan();
      await createUser({
         subscription: {
            plan: 'pro',
      ...
      const first = await postWebhook(invoiceEvent);
      const second = await postWebhook({
         ...invoiceEvent,
         id: 'evt_invoice_paid_repeat_2'
      });

      expect(first.status).toBe(200);
      expect(second.status).toBe(200);

      const notifications = await Notification.find({ 'data.invoiceId': 'in_test_789' });
